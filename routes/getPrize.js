...
router.get("/", (req, res) => {
  ...
  try {
    ...
    if (lotto_type_id != undefined && date != undefined) {
      var sql = `SELECT p.prize3top, p.prize2bottom, lt.lotto_type_name, lt.lotto_type_id, p.prize_time FROM prize as p LEFT JOIN lotto_type as lt ON p.lotto_type_id = lt.lotto_type_id WHERE lt.lotto_type_name IN ("รัฐบาลไทย","หวยออมสิน") AND p.status = 1 ORDER BY p.prize_time DESC LIMIT 2`;
      connection.query(sql, ["รัฐบาลไทย"], (error, resultThai, fields) => {
        var sql = `SELECT lt.lotto_type_id, lt.type_id, lt.lotto_type_name, lt.lotto_type_img, lt.closing_time, IFNULL(( SELECT p.prize3top FROM prize as p WHERE p.prize_time = ? AND p.lotto_type_id = lt.lotto_type_id AND p.status = 1 ORDER BY p.prize_id DESC LIMIT 1), 'xxx') AS prize3top, IFNULL(( SELECT p.prize2bottom FROM prize as p WHERE p.prize_time = ? AND p.lotto_type_id = lt.lotto_type_id AND p.status = 1 AND lt.lotto_type_id = ? ORDER BY p.prize_id DESC LIMIT 1), 'xx') AS prize2bottom FROM lotto_type AS lt ORDER BY closing_time ASC`;
        connection.query(
          sql,
      ...
      connection.query(sql, ["รัฐบาลไทย"], (error, resultThai, fields) => {
        ...
        connection.query(sql, [date, date], (error, result, fields) => {
          console.log(result);
          try {
            if (result === undefined) {
              return res
